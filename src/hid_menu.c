...
static void USBH_PS3Demo(USBH_HandleTypeDef *phost);
...
void HID_MenuProcess(void)
{
	...
	case APPLI_HID_START:
		if(Appli_FS_state == APPLICATION_FS_READY )
		{
			...
			else if(USBH_HID_GetDeviceType(&hUSBHost_FS) == HID_PS3)
			{
				demo.ps3_state = HID_PS3_IDLE;
				demo.hid_state = APPLI_HID_PS3;
...
/**
 * @brief  Main routine for PS3 application
 * @param  phost: Host handle
 * @retval None
 */
static void USBH_PS3Demo(USBH_HandleTypeDef *phost)
{
	HID_PS3_Info_TypeDef *m_pinfo;
