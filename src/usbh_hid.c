...
extern USBH_StatusTypeDef USBH_HID_PS3Init(USBH_HandleTypeDef *phost);
...
static USBH_StatusTypeDef USBH_HID_InterfaceInit (USBH_HandleTypeDef *phost)
{
	...
	{
		interface = USBH_FindInterface(phost, phost->pActiveClass->ClassCode, PS3_BOOT_CODE, 0xFF);
		if(interface == 0xFF) /* No Valid Interface */
		{
			status = USBH_FAIL;
	...
	else if(phost->device.CfgDesc.Itf_Desc[phost->device.current_interface].bInterfaceProtocol  == HID_PS3_BOOT_CODE)
	{
		USBH_UsrLog ("DualShock3 found!");
		HID_Handle->Init =  USBH_HID_PS3Init;
	}
	else
	{
